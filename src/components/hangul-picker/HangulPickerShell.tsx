...
import { HangulPickerGame } from "@/components/hangul-picker/HangulPickerGame";
import { VersionSelector } from "@/components/hangul-picker/VersionSelector";
import { versionInfo } from "@/lib/hangul-picker/data";
import { HangulPickerVersion } from "@/lib/hangul-picker/types";
import { defaultKidsBgm } from "@/lib/shared/media";

export function HangulPickerShell() {
  const [selectedVersion, setSelectedVersion] = useState<HangulPickerVersion | null>(null);
  ...
  const handleStartVersion = (version: HangulPickerVersion) => {
    bgmArmedRef.current = true;
      ...
      <div className="mx-auto flex w-full max-w-7xl flex-col gap-6">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div>
            ...
            <h1 className="mt-1 text-3xl font-black text-slate-900 sm:text-4xl">한글 그림 맞추기</h1>
          </div>
          <Link
            href="/"
        ...
        {isPlaying && selectedVersion ? (
          <>
            ...
            <HangulPickerGame key={`${selectedVersion}-${gameSessionKey}`} version={selectedVersion} />
          </>
        ) : (
      ...
      <section className="fixed inset-x-0 bottom-0 z-20 border-t border-white/70 bg-white/90 px-4 py-3 shadow-[0_-12px_40px_rgba(52,84,104,0.14)] backdrop-blur sm:px-6">
        <div className="mx-auto flex w-full max-w-7xl flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
          <div>
            ...
            <p className="mt-1 text-sm text-slate-500">모든 어린이게임 기본 음악으로 같은 노래를 쓰도록 맞췄어요.</p>
          </div>
