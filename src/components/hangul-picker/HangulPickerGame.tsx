...
import { versionInfo } from "@/lib/hangul-picker/data";
import {
  accuracy,
  advanceSession,
  ...
  readBestScores,
  writeBestScores,
} from "@/lib/hangul-picker/game-utils";
import { BestScores, HangulGameSession, HangulPickerVersion } from "@/lib/hangul-picker/types";
...
type HangulPickerGameProps = {
  version: HangulPickerVersion;
};

export function HangulPickerGame({ version }: HangulPickerGameProps) {
  const info = versionInfo[version];
  const [session, setSession] = useState<HangulGameSession>(() => createInitialSession(version));
  ...
  const secondsLeft = deadlineAt === null ? null : Math.max(0, Math.ceil((deadlineAt - timeNow) / 1000));
  ...
  const handleTimeout = useCallback(() => {
    ...
    if (nextMistakes >= maxMistakes) {
      ...
      showToastThen({
        tone: "error",
        message: `시간 초과! 정답은 "${question.target.word}"예요. 다섯 번 틀려서 게임 종료예요.`,
      });
      return;
    }
    ...
    showToastThen(
      {
        tone: "error",
        message: `시간 초과! 정답은 "${question.target.word}"예요.`,
      },
      moveToNextQuestion
    );
  }, [interactionLocked, mistakes, moveToNextQuestion, question.target.word, showToastThen]);
  ...
  const handleChoice = (choiceId: string) => {
    ...
    const correct = choiceId === question.target.id;
    setQuestionCount((prev) => prev + 1);
    ...
    if (correct) {
      ...
      setBestScores((prev) => (nextScoreValue > prev.practice ? { ...prev, practice: nextScoreValue } : prev));

      showToastThen(
        {
          tone: "success",
          message: `${question.target.word} 찾기 성공!${nextStreak >= 2 ? ` ${nextStreak}콤보!` : ""}`,
        },
        moveToNextQuestion
      );
      return;
    ...
    if (nextMistakes >= maxMistakes) {
      ...
      showToastThen({
        tone: "error",
        message: `아쉬워요! 정답은 "${question.target.word}"예요. 다섯 번 틀려서 게임 종료예요.`,
      });
      return;
    }
    ...
    showToastThen(
      {
        tone: "error",
        message: `아쉬워요! 정답은 "${question.target.word}"예요.`,
      },
      moveToNextQuestion
    );
  };
  ...
  return (
    <section className="grid gap-5">
      ...
      <div className="relative rounded-[2rem] bg-white p-6 shadow-[0_18px_60px_rgba(52,84,104,0.12)] ring-1 ring-slate-100 sm:p-8 lg:p-10">
        <div className={`rounded-[2rem] bg-gradient-to-br ${info.softPanelClassName} p-5 ring-1 ring-white/80 sm:p-6`}>
          <div className="flex flex-wrap items-start justify-between gap-4">
            <div>
              ...
              <h2 className="mt-3 text-3xl font-black text-slate-900 sm:text-4xl">이 그림은 무슨 글자일까요?</h2>
          ...
          <div className="mt-6 flex justify-center">
            <div className="flex h-40 w-40 items-center justify-center rounded-[2rem] bg-white text-8xl shadow-inner ring-1 ring-white/80 sm:h-48 sm:w-48">
        ...
        <div
          className={`mt-8 grid gap-4 ${
            info.choiceCount >= 5 ? "sm:grid-cols-2 xl:grid-cols-3" : "sm:grid-cols-2 xl:grid-cols-3"
          }`}
        >
          {question.choices.map((choice) => (
            <button
              key={`${session.round}-${choice.id}`}
              type="button"
        ...
        <div className="mt-8 rounded-[1.5rem] bg-slate-50 px-5 py-4 text-sm leading-7 text-slate-600 ring-1 ring-slate-100">
          <strong className="font-black text-slate-900">순서 규칙:</strong> 낱말은 한 바퀴를 전부 돌 때까지 중복 없이 나오고, 다 나온 뒤에만 다음 랜덤 사이클로 넘어가요.
        </div>
      ...
      {isFinished ? (
        <ResultPanel
          title={`${info.shortLabel} 한글 그림 맞추기 종료!`}
          body={
            info.timeLimitSeconds
              ? "나린이 버전은 8초 안에 빠르게 찾는 도전 모드예요. 위쪽 다시 시작 버튼으로 바로 또 도전할 수 있어요."
