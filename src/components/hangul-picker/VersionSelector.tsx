...
type VersionSelectorProps = {
  onSelect: (version: HangulPickerVersion) => void;
};
    ...
    <section className="rounded-[2rem] bg-white/90 p-6 shadow-[0_24px_80px_rgba(40,68,87,0.14)] ring-1 ring-white/70 backdrop-blur sm:p-8">
      <div className="max-w-3xl">
        ...
        <p className="mt-4 text-lg leading-8 text-slate-600">
          버전을 큰 카드로 눌러서 바로 시작해요. 그림을 보고 알맞은 글자를 찾아보세요.
        </p>
      </div>

      <div className="mt-8 grid gap-5 lg:grid-cols-2">
        {(Object.entries(versionInfo) as [HangulPickerVersion, (typeof versionInfo)[HangulPickerVersion]][]).map(
          ([version, info]) => (
            <button
              key={version}
              type="button"
