...
function AnimalCard({ pair, matchedFoodId, disabled }: AnimalCardProps) {
  ...
  return (
    <div
      ref={setNodeRef}
      className={`rounded-[2rem] bg-white/92 p-5 shadow-[0_16px_40px_rgba(52,84,104,0.12)] ring-1 transition ${
        matched
          ? "ring-emerald-200 bg-emerald-50/90"
          : isOver
            ? "scale-[1.02] ring-sky-300 shadow-2xl"
            : "ring-white/80"
      }`}
    >
      <div className="flex items-center justify-between gap-3">
        <div>
          <p className="text-xs font-semibold uppercase tracking-[0.25em] text-slate-400">동물 친구</p>
          <h2 className="mt-2 text-2xl font-black text-slate-900">{pair.animalName}</h2>
        </div>
        <div className="text-5xl sm:text-6xl">{pair.animalEmoji}</div>
      </div>

      <div
        className={`mt-5 rounded-[1.5rem] border-2 border-dashed px-4 py-5 text-center transition ${
          matched
      ...
      >
        {matched ? (
          <>
            <div className="text-4xl">{pair.foodEmoji}</div>
            <p className="mt-2 text-lg font-black">{pair.foodName} 냠냠 완료!</p>
          </>
        ) : (
          <>
            <p className="text-sm font-bold">여기로 먹이를 끌어다 주세요</p>
            <p className="mt-2 text-sm leading-6">{pair.hint}</p>
          </>
        )}
      </div>
    </div>
...
export function AnimalFeedingGameShell() {
  ...
  return (
    <main className="min-h-screen bg-[radial-gradient(circle_at_top,_#fff9db,_#eef8ff_48%,_#eef7ef)] px-4 pb-40 pt-6 text-slate-800 sm:px-6 lg:px-8">
      <div className="mx-auto flex w-full max-w-7xl flex-col gap-6">
            ...
            <div className="flex h-full flex-col gap-4 rounded-[calc(2rem-2px)] bg-white/95 p-6 sm:p-7">
              <div className="flex flex-wrap items-center justify-between gap-3">
                <div>
                  ...
                  <h2 className="mt-2 text-2xl font-black text-slate-900">동물에게 맞는 먹이를 끌어다 주세요</h2>
                </div>
              ...
              <p className="text-base leading-7 text-slate-600">
                먹이 카드를 아래에서 끌어와 동물 친구에게 주세요. 맞는 먹이를 주면 점수가 올라가고, 모두 먹이면
                게임이 끝나요.
              </p>
            </div>
          </div>
        ...
        <DndContext sensors={sensors} onDragEnd={handleDragEnd}>
          <section className="grid gap-4 sm:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-5">
            {animalFoodPairs.map((pair) => (
              <AnimalCard key={pair.id} pair={pair} matchedFoodId={matchedByAnimal[pair.id]} disabled={interactionLocked} />
            ))}
          </section>
          ...
          <section className="rounded-[2rem] bg-white/88 p-5 shadow-[0_18px_60px_rgba(52,84,104,0.12)] ring-1 ring-white/80 sm:p-6">
            <div className="flex flex-wrap items-center justify-between gap-3">
              <div>
                ...
                <h2 className="mt-2 text-2xl font-black text-slate-900">아래 먹이를 동물에게 주세요</h2>
              </div>
