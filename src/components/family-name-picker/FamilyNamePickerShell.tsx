...
import { VersionSelector } from "@/components/family-name-picker/VersionSelector";
...
export function FamilyNamePickerShell() {
  const [selectedVersion, setSelectedVersion] = useState<FamilyNameVersion | null>(null);
  ...
  const handleStartVersion = (version: FamilyNameVersion) => {
    bgmArmedRef.current = true;
      ...
      <div className="mx-auto flex w-full max-w-7xl flex-col gap-6">
        ...
        {isPlaying && selectedVersion ? (
          <>
            ...
            <FamilyNamePickerGame key={`${selectedVersion}-${gameSessionKey}`} version={selectedVersion} />
          </>
        ) : (
